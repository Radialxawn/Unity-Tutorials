...
public class GameController : MonoBehaviour {
	...
	private static void GameOver() {
		...
		// save high score
		if (In2DMode) {
			if (Score > PlayerData.BestScore.Mode2D.Get()) {
				PlayerData.BestScore.Mode2D.Set(Score);
			}
		} else {
			if (Score > PlayerData.BestScore.Mode3D.Get()) {
				PlayerData.BestScore.Mode3D.Set(Score);
			}
		}
